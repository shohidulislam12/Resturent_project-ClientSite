 ...
 const axiousSecure  =useAxiouSecure()
    ...
    const onSubmit = async(data) => {
        ...
        if(res.data.success){
            ...
            // set database
            const menu=await axiousSecure.post('/menu',menuItem)
            console.log("menuitem database",menu.data)
            if(menu.data.acknowledged===true){
               // reset()
                console.log('sucess')
                toast.success("item add Sucess")
            }
        }
...
<label className="form-control my-6 w-full gap-4">
  ...
  <input  
  {...register("price")}
   type="number" placeholder="price here" 
   className="input input-bordered w-full " 
   required />
...
{/* sm */}<label className="form-control  my-5 w-full max-w-xs">
  <div className="label">
    <span className="label-text">Chose a file</span>
   
  </div>
  <input 
  {...register("image")} type="file" className="file-input file-input-bordered w-full max-w-xs" required />

</label>

<button className='btn bg-orange-400 m-2 '>Add Item <FaUtensils></FaUtensils></button>
    </form>
           </div>
        </div>
    );
